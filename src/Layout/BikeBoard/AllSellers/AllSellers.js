...
const AllSellers = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch(`https://assignment-12-server-murex.vercel.app/users/allSellers`);
    ...
    const handleDeleteSellers = id =>{
        ...
        if(proceed){
            fetch( `https://assignment-12-server-murex.vercel.app/users/allSellers/${id}`, {
                method: 'DELETE'
            })
            .then(res => res.json())
            .then(data => {
...
const handleMakeAdminVerify = id => {
    fetch(`https://assignment-12-server-murex.vercel.app/users/admin/${id}`, {
        method: 'PUT', 
        headers: {
